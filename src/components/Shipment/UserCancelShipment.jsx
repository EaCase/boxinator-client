...
import MuiColorPicker from "../common/forms/MuiColorPicker";
    ...
    const UserCancelShipment = ({ oneShipment, tiers, countries, handleUpdate }) => {
            const initialValues = {
            recipient: oneShipment.recipient,
            tier: tiers.find((item) => item.id === oneShipment.boxTier.id),
            country: countries.find((item) => item.id === oneShipment.country.id),
            };
          ...
          <Box>
            <Box display="flex" mb={3}>
              <Typography margin="auto" variant="h4">
                Edit your shipment information
              </Typography>
            </Box>
...
export default UserCancelShipment;